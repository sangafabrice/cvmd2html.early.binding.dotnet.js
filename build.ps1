...
Function Build-MarkdownToHtmlShortcut {
  ...
  Remove-Item ($MsAdoDllPath = "$PSScriptRoot\Interop.ADODB.dll") -ErrorAction SilentlyContinue
  ...
  & "$PSScriptRoot\TlbImp.exe" /nologo /silent 'C:\Program Files\Common Files\System\ado\msado15.dll' /out:$MsAdoDllPath /namespace:ADODB
  ...
  jsc.exe /nologo /target:$($DebugPreference -eq 'Continue' ? 'exe':'winexe') /win32res:$TargetInfoResFile /reference:$MsAdoDllPath /reference:$MshtmlDllPathCopy /reference:$SWbemDllPath /reference:$WshDllPath /out:$ConvertExe "$PSScriptRoot\AssemblyInfo.js" "$PSScriptRoot\StdRegProv.js" $(Set-ConvertMd2HtmlExtension '.js')
  $Env:Path = $EnvPath
